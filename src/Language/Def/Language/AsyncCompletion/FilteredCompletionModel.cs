...
namespace Microsoft.VisualStudio.Language.Intellisense.AsyncCompletion
{
    ...
    public sealed class FilteredCompletionModel
    {
        ...
        /// </summary>
        public CompletionItemSelection SelectionMode { get; }
        ...
        public FilteredCompletionModel(ImmutableArray<CompletionItemWithHighlight> items, int selectedItemIndex)
            : this(items, selectedItemIndex, ImmutableArray<CompletionFilterWithState>.Empty, selectionMode: CompletionItemSelection.NoChange, centerSelection: true, uniqueItem: null)
        {
        }
        ...
        public FilteredCompletionModel(ImmutableArray<CompletionItemWithHighlight> items, int selectedItemIndex, ImmutableArray<CompletionFilterWithState> filters)
            : this(items, selectedItemIndex, filters, selectionMode: CompletionItemSelection.NoChange, centerSelection: true, uniqueItem: null)
        {
        }
        ...
        /// <param name="selectedItemIndex">Recommended item index to select.</param>
        /// <param name="filters">Completion filters with their availability and selection state.</param>
        /// <param name="selectionMode">Allows <see cref="IAsyncCompletionItemManager"/> to influence the selection mode.</param>
        /// <param name="uniqueItem">Provides <see cref="CompletionItem"/> to commit using "commit if unique" command despite displaying more than one item.</param>
        public FilteredCompletionModel(
            ...
            CompletionItemSelection selectionMode,
